...
✅ **DecisionEngine v2** (`server/`) — built ALONGSIDE the old keyword matcher. ⚠️ **NOT wired in yet**: `POST /api/decisions/query` still calls `decideRestaurants()` in `services/decisionEngine.ts`. Swapping it in is a separate, deliberate step.
...
👉 **NEXT UP — wire DecisionEngine v2 into `POST /api/decisions/query`.** Everything it needs exists and is tested; the route still calls the old keyword matcher on purpose, pending review. Wiring it means: parse the Decide flow's filters into an `EngineContext` (the mobile client currently sends a prompt STRING like "Near JBR, Lebanese, Dine In, Casual" — either parse it or change the client to send structured filters, the latter being cleaner), load `getTasteWeights()` / `getRecentSelections()` / `getPickRates()`, call `decide()`, then `logDecision()`. Keep `decideRestaurants()` around until v2 has been live a while.
