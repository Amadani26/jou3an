...
ResultCard.tsx — decision result card, redesigned premium/minimal (bg #111111, radius 16, no border, subtle shadow, sizes to content): full-width placeholder food image header (height 180, top corners radius 12, `getPlaceholderImage(rank-1)`) with the rank (48px red #E63946) overlaid bottom-left over a dark gradient; then name (20/700), meta line (cuisine · AED price in amber #F4A261 · pin+area), up to 2 tag pills (#1a1a1a/#2a2a2a border), and a pinned 3-col action row (Directions/Reserve/Order — icon 20 #666, label 10 #555 uppercase; each col is a flex:1 View wrapper so they split evenly; Directions has paddingLeft 16). Score bars REMOVED (revisit when AI engine is live; `deriveScores` in lib/api is now unused). Tap (onPress) = select (red tint + glow + checkmark; other cards dim to 0.5), long-press (400ms) = RestaurantDetailSheet. Props: `onSelect`, `selected`, `dimmed`.
