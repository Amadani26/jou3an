...
✅ **Restaurant import pipeline** — discovery → human curation → import, built on the existing Places sync. Three scripts, all in `server/`:
  - **`npm run discover -- --area "Dubai Marina" [--cuisine "Japanese"]`** (`src/scripts/discoverPlaces.ts`) — paged Places Text Search ("restaurants in {area}, Dubai", or "{cuisine} restaurants in …"), filtered to `rating ≥ --min-rating` (default **3.8**) AND `userRatingCount ≥ --min-reviews` (default **400**) AND `businessStatus === 'OPERATIONAL'`. Also excludes place ids already in the DB and any id in the rejected list. Writes `server/imports/<area>[-<cuisine>].candidates.json` and prints a table plus the **API call count** and a per-reason breakdown of what was filtered out. ⚠️ **It never writes to the database** — the output is a proposal. `--max-pages` (default 3, 20 results/page) and `--out` are also available. Missing rating/review data counts as FAILING the threshold, not passing.
  - **Curation is manual and deliberate**: `cp imports/<x>.candidates.json imports/<x>.approved.json`, then delete the rows you don't want.
  - **`npm run import -- --file server/imports/<x>.approved.json`** (`src/scripts/importPlaces.ts`) — creates a Restaurant per row: name, **pinned `googlePlaceId`** (the id discovery matched, so Text Search can never second-guess it), cuisine from `cuisineFromTypes`, area from `areaFromPlace`, price from `priceRangeFor`, `isActive: true`, `ratingScore` = `NEUTRAL_RATING_SCORE` (7.5). Then runs `syncRestaurantPlaces()` on just those rows for photos/rating/coords/hours. **IDEMPOTENT**: a place id already in the DB is skipped, so re-running the same file is free. `--dry-run` reports without writing; `--no-sync` creates rows without enrichment.
  - **Rejection memory**: after a real import the candidates file is diffed against the approved file and every dropped place id is appended to `imports/rejected.json` (with a note saying which file it was dropped from), so discovery stops proposing it. The candidates file is found automatically from the approved filename; `--candidates <path>` overrides. **`npm run unreject -- --placeId <id>`** removes one (`--list` shows them all) — a rejection is a convenience, not a permanent verdict.
  - **Mapping** (`src/lib/importMapping.ts`, pure + unit-tested): `TYPE_TO_CUISINE` maps ~40 Google place types onto our cuisine strings (sushi/ramen both collapse to "Japanese"; vegan/vegetarian to "Healthy"), falling back to **"International"**; generic types (`restaurant`, `food`, `establishment`…) are skipped when scanning secondary types. ⚠️ `areaFromPlace` checks the **formatted address FIRST** and only then coordinates — DIFC, Downtown and Business Bay sit within ~2 km of each other, so a nearest-centroid guess between them is close to a coin flip. Centroid radii are deliberately tight (1.1–2.2 km) because a wrong area is worse than `OTHER`. JBR and Bluewaters map to MARINA, matching how the seed already files them. `PRICE_BANDS` maps Google's price enum to AED-per-head ranges, defaulting to 45–100 when Google has no signal.
  - **Rate limiting**: 1.2s between discovery pages, 400ms between per-row Places calls during import, 300ms between rows in `sync:places`.
  - ⚠️ **`server/imports/` is GITIGNORED.** Candidate/approved files are one curation session's scratch and `rejected.json` is a local memory of what a human declined — none of it is code, and committing it would make rejections someone else's problem.
  - Test run (2026-09-20, Dubai Marina, no cuisine filter): **43 candidates from 60 results in 3 API calls**, 16 dropped for too few reviews, SALT correctly excluded as already in the DB. Nothing was imported.
  - ⚠️ Worth knowing before a real import: a lot of Dubai Marina places come back with `primaryType: 'restaurant'` and therefore map to **"International"**. The cuisine is then only as good as the fallback — expect to hand-correct `cuisineType` after importing, or extend `TYPE_TO_CUISINE`.
...
Restaurant import pipeline (see the full entry under What's Built):
```bash
cd server
npm run discover -- --area "Dubai Marina"                        # candidates file, writes NO DB rows
npm run discover -- --area "Dubai Marina" --cuisine "Japanese"   # narrowed
npm run discover -- --area "JLT" --min-rating 4.2 --min-reviews 800 --max-pages 5
cp imports/dubai-marina.candidates.json imports/dubai-marina.approved.json   # then DELETE rows you don't want
npm run import -- --file server/imports/dubai-marina.approved.json           # create + sync just the new rows
npm run import -- --file server/imports/dubai-marina.approved.json --dry-run # report only, writes nothing
npm run unreject -- --list                                       # what has been declined
npm run unreject -- --placeId ChIJ…                              # let discovery propose it again
```
Google Places sync: `cd server && npm run sync:places` (Text Search "{name} {area} Dubai" → Place Details; stores up to 6 photoRefs, lat/lng, googleRating, googleSyncedAt; prints a matched/pinned/unmatched table and flags places matched under a different Google name). Safe to re-run — it overwrites the Places fields only. `--only <id | name fragment>` syncs a single restaurant. The per-row work now lives in `server/src/services/placesSync.ts` (`syncRestaurantPlaces`), shared with the import pipeline so new rows get identical enrichment without re-billing the whole catalogue.
