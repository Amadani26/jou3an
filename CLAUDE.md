...
- index.tsx — Home: INFORMATIONAL, ordered so a first-time user understands the product before the CTA. Top to bottom: (1) tight hero — the 48px red ج logo mark + one line "Hungry? We decide for you." (`decide` in red italic, `adjustsFontSizeToFit` + `numberOfLines={1}`); (2) a "how it works" strip — 3 side-by-side `StepCard`s (bg #111, 1px #1C1C1C, radius 14, ~66–80 tall) each with a red #E63946 DMSans_800ExtraBold number, a 12px bold label and one 10px #504B47 sub-line — 01 "Tell us your vibe" / 02 "Get exactly 3" / 03 "Eat"; (3) the bouncing "Decide for me →" CTA (unchanged Reanimated spring loop); (4) Daily Top 3 last (pulsing dot + date + theme label, skeletons while loading, static FALLBACK_PICKS on failure). The old 52px HUNGRY?/WE decide headline, the Arabic tagline and the paragraph were removed. Spacing is tuned so the hero + steps + CTA + the first two daily cards all fit above the tab bar on an iPhone 15-class screen (~769pt of viewport), with only the third card needing a short scroll — `DailyCard` is passed `compact` here (100px image instead of 160, tighter padding, rank 30 instead of 38) to make that fit. Trim whitespace, not the CTA, if this needs re-tuning.
...
DailyCard.tsx — daily pick card. Takes `compact` (Home passes it: 100px image, tighter padding, 30px rank) — default is the full-size card. Full-width real Places photo header (height 160 / 100 compact, top corners radius 24 to match card) with the rank overlaid bottom-left over a dark gradient, then name, cuisine · area, price below. Image = `getPlaceholderImage(rank - 1)`.
...
👉 NEXT PHASE (in order): Railway deployment (backend) → Vercel deployment (web landing page) → Phase 4 AI activation → geo-filtering for nearby restaurants (lat/lng now exist on Restaurant, so this is unblocked)
❌ Railway deployment (backend) — see "Railway Environment Variables" section below
❌ Vercel deployment (web landing page)
❌ Geo-filtering for nearby restaurants — `GET /api/restaurants/nearby` accepts lat/lng/radius but still returns all active (no distance filtering yet); Decide "Nearby" and Tinder pass coords but they're unused server-side. UNBLOCKED: Restaurant.lat/lng are now populated by the Places sync.
...
Then: Phase 4 AI activation (swap decisionEngine.ts placeholder once ANTHROPIC_API_KEY is set) → real geo-filtering for `/api/restaurants/nearby` (lat/lng are populated; just needs a distance filter). The Places data is verified: `Sushi Counter` was replaced by the real **Sushi Art** (DIFC, pinned), `Ravi Restaurant` is pinned to the **Satwa** original, and `Operation Falafel` is now seeded as MARINA to match the JBR coordinates Places returns.
